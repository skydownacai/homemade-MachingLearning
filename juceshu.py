    ...
    def Decision(self,D,cost_function = 1):
        ...
        if cost_function == 1:
                 ...
                 for cut in cuts:
                     ...
                     for item in D:
                         if float(item[key]) >= cut:
                             ...
                             smaller.append(item)
                     '''划分为大于等于与小于两个子集'''
                     this_gain = entropy_of_father - (len(bigger))/len(D)*self.entropy(bigger) - (len(smaller)/len(D))*self.entropy(smaller)
            ...
            if max_gain < max_gain_contious:
                return (max_gain_contious,features[max_gain_index_contious],max_gain_partition_label_contious,max_gain_partition_contious)
        ...
        if cost_function == 2:
            ...
            if mini_gini_contious < mini_gini:
                return (mini_gini_contious,features[mini_gini_index_contious],mini_gini_partition_label_contious,mini_gini_partition_contious)
        ...
        while True:
            ...
            for i in range(len(self.Tree[count])):
                ...
                if Decision == 0:
                   ...
                   print('第{}层第{}节点分裂:属性{};分裂标签{},GINI{}'.format(count+1,i+1,Decision[1],Decision[2],Decision[0]))
